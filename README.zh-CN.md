...
- **LL-HLS** — 低延迟 HLS，支持部分分片、阻塞式播放列表刷新和 fMP4 容器
- **多协议集群转发** — 支持 RTMP、SRT、RTSP、RTP 多协议转推和按需回源拉流，支持 HTTP 调度回调；SRT 低延迟转发适用于专业直播，RTMP 通用转发适用于 CDN 级联
- **ICE Lite** — WebRTC 可选 ICE Lite 模式：服务端跳过候选收集和连通性检测，公网服务器 ICE 协商瞬间完成
- **慢消费者保护** — 基于 EWMA 的延迟检测，渐进式丢帧防止级联卡顿
- **GCC 拥塞控制** — WebRTC WHEP 发送端带宽估计，自适应码率平滑发送；初始码率根据流实际码率自动调整
- **限流** — 基于令牌桶的 IP 级连接限流，防止连接洪泛
...
| `cluster` | 多协议转推（RTMP/SRT/RTSP/RTP）和回源拉流，支持调度器 |
| `metrics` | Prometheus 监控端点（默认 `:9090`） |
| `limits` | 全局连接数、流数、订阅者数限制 |
| `tls` | TLS 证书和密钥配置 |
| `stream` | GOP 缓存、环形缓冲区、空闲超时、慢消费者设置 |
...
- **[集群部署指南](docs/cluster-guide.zh-CN.md)** — 多节点集群部署，包含 SRT 低延迟和 RTMP 通用方案示例配置

## 路线图

- [x] TLS / HTTPS 支持
- [x] SRT 协议
- [x] 多协议集群转发（RTMP、SRT、RTSP、RTP）
- [x] WebRTC ICE Lite
- [x] WebSocket 通知
- [x] Prometheus 监控指标
- [x] LL-HLS（部分分片 + 阻塞式刷新）
