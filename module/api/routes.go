...
func registerRoutes(mux *http.ServeMux, s *core.Server, audit *AuditStore) {
	...
	mux.HandleFunc("GET /api/v1/server/config/history", h.handleConfigHistory)
	mux.HandleFunc("GET /api/v1/server/config/history/{revision}", h.handleConfigHistoryDocument)
	mux.HandleFunc("POST /api/v1/server/config/rollback", h.handleConfigRollback)
