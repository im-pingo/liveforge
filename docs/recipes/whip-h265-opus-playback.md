...
The `audiocodec` profile is important for FLV- and TS-based outputs: Opus is passed through by compatible fMP4/WHEP paths, while incompatible output containers request a shared AAC transcode. HTTP muxers start a combined transcode reader at the cached GOP source position, so converted audio and live source video continue after the cached keyframe without a first-frame freeze; cached video is filtered by video DTS to avoid duplication. Without the tag, those outputs may be video-only instead of carrying transcoded audio.
...
- HLS or LL-HLS pauses after cached playback: verify the HTTP muxer uses the combined transcode reader, which contributes target audio history and live source video from the cached GOP source position; cached video is filtered only by video DTS. The bundled Hls.js requires one completed segment in its initial manifest, then consumes low-latency parts; the server must not wait for the old three-segment buffer. The live reader begins at the atomic snapshot cursor, so no cross-track DTS watermark should discard a valid frame.
