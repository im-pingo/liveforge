...
The `audiocodec` profile is important for FLV- and TS-based outputs: Opus is passed through by compatible fMP4/WHEP paths, while incompatible output containers request a shared AAC transcode. Without the tag, those outputs may be video-only instead of carrying transcoded audio.
...
- HLS or LL-HLS pauses after cached playback: verify live interframes after the atomic cache snapshot remain in the same keyframe-bounded segment. The bundled Hls.js requires one completed segment in its initial manifest, then consumes low-latency parts; the server must not wait for the old three-segment buffer.
