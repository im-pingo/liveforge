...
- **GB28181 lifecycle and RTP port ownership** — Inbound device INVITEs complete publish-start admission before a final 2xx response; backpressure returns non-2xx and reclaims the publisher, session, newly created stream, sockets, and ports without an unmatched publish-stop. Receive Lab and self-test paths bind both RTP/RTCP sockets while reserving a pair, skip externally occupied pairs, and release them exactly once. Accepted live/playback dialogs have one managed ACK/BYE/close owner across rollback and normal teardown
