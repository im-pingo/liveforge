...
// FrameProvenance identifies whether an audio frame came directly from the
// publisher or was generated by an audio transform. It is internal routing
// metadata and is not serialized into media containers.
type FrameProvenance uint8

const (
	FrameProvenanceUnknown FrameProvenance = iota
	FrameProvenanceSource
	FrameProvenanceTranscoded
)
...
type AVFrame struct {
	...
	// AudioCodecEpoch increments when one publisher generation changes its
	// source audio codec. AudioProvenance distinguishes direct source frames
	// from generated sequence headers and encoded transform output.
	AudioCodecEpoch uint64
	AudioProvenance FrameProvenance
}
